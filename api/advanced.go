...
func (s *Server) GetAdvancedPractice(c *fiber.Ctx) error {
	...
	if payload.Identifier == "" {
		...
		advOC, err := s.makeAdvancedChartToRef(db.OneH, nextPair, c.Context())
		if err != nil {
			s.logger.Err(err).Msg("cannot get advanced practice chart")
			return c.SendStatus(fiber.StatusInternalServerError)
		}
		s.logger.Debug().Str("name", advOC.Name).Str("interval", db.OneH).Str("From", utilities.TransMilli(advOC.PvDatas[len(advOC.PvDatas)-1].Time)).Str("To", utilities.TransMilli(advOC.PvDatas[0].Time)).Msg("get advanced practice chart.")
		return c.Status(fiber.StatusOK).JSON(advOC)
	}
